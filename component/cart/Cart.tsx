...
const Cart = () => {
  ...
  return (
    <div>
      ...
      <div>
        {cartItems.map((cart) => <div key={cart.id}> <Image key={cart.id} alt={cart.title}src={cart.image} width={50} height={50} layout={'fixed'}/>  <button  key={cart.id} onClick={() => onRemove(cart.id)}>delete</button> </div>)}
      </div>
